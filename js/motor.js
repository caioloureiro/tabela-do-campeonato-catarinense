...
function verificarCarregamentoCompleto() {
	if (ajaxCompleted >= ajaxRequests && ajaxRequests > 0) {
		console.log("✓ Todos os AJAX foram processados");
...
$(".link-artilheiro").html(
	'<a href="artilheiro.html?sumula=1&iAno=' +
		iAno +
		"&iCampeonato=" +
		iCampeonato +
		'" target="_blank">VER TABELA COMPLETA</a>',
);
$(".link-goleiro").html(
	'<a href="goleiro.html?sumula=1&iAno=' +
		iAno +
		"&iCampeonato=" +
		iCampeonato +
		'" target="_blank">VER TABELA COMPLETA</a>',
...
document.onkeydown = function (evt) {
	...
	if (isEscape) {
		/*SEMPRE QUE SE PRESSIONAR ESC FAZ O QUE ESTA ABAIXO*/
		console.log("ESC");
		$(".loading").removeClass("on");
		$(".escurecer").removeClass("on");
		$(".inscricoes").removeClass("on");
...
$(document).ready(function () {
	// Failsafe: remover loading após 10 segundos mesmo se algo falhar
	setTimeout(function () {
		$(".loading").removeClass("on");
	}, 10000);
	...
	$.ajax({
		...
		success: function (retorno) {
			...
			console.log(
				"5. campeonato.json - campeonato - Identifica os dados do campeonato através da URL => JSON:",
			);
			console.log(campeonato);

			console.log("5.0.1. campeonato.json - URL de retorno:");
			console.log(
				"http://egol.fcf.com.br/sesp/WsEgol.wso/Get_IdCampeonato/JSON/debug?sCampeonato=" +
					iCampeonato,
			);

			for (i = 0; i < campeonato.length; i++) {
				$(".menu-select select").append(
					'<option class="menu-select-' +
						campeonato[i].iAno +
						'" value="' +
						campeonato[i].iAno +
						'">' +
				...
				if (campeonato[i].iAno == iAno) {
					...
					console.log(
						"5.1. campeonato.json - campeonato[i].sCampeonato - Monta o select do menu => STRING:",
					);
			...
			console.log(
				"5.2. campeonato.json - contador - A contagem foi modificada para o próximo passo => NUMBER:",
			);
			console.log(contador);
		},
	});
	...
	$.ajax({
		...
		success: function (retorno) {
			...
			console.log(
				"6. tabela.json - tabela - Identifica os dados da tabela => JSON:",
			);
			console.log(tabela);

			console.log("6.0.1. tabela.json - URL de retorno:");
			console.log(
				"http://egol.fcf.com.br/sesp/WsEgol.wso/Get_Campeonato/JSON/debug?iAno=" +
					iAno +
					"&iCampeonato=" +
					iCampeonato,
			);
			...
			$(".principal-tabela-carrossel").addClass(
				"principal-tabela-carrossel-turnos",
			);
			...
			if (tabela.length >= 0 && tabela[0]) {
				...
				$(".principal-tabela-carrossel").append(
					"" +
						...
						'		<div class="principal-jogos-barra-txt">' +
						tabela[0].sNome_Fase +
						"</div>" +
						"	</div>" +
				...
				for (i = 0; i < tabela[0].Grupo[0].Tabela.length; i++) {
					...
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 1) {
						pos01 = tabela[0].Grupo[0].Tabela[i];
					}
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 2) {
						pos02 = tabela[0].Grupo[0].Tabela[i];
					}
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 3) {
						pos03 = tabela[0].Grupo[0].Tabela[i];
					}
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 4) {
						pos04 = tabela[0].Grupo[0].Tabela[i];
					}
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 5) {
						pos05 = tabela[0].Grupo[0].Tabela[i];
					}
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 6) {
						pos06 = tabela[0].Grupo[0].Tabela[i];
					}
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 7) {
						pos07 = tabela[0].Grupo[0].Tabela[i];
					}
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 8) {
						pos08 = tabela[0].Grupo[0].Tabela[i];
					}
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 9) {
						pos09 = tabela[0].Grupo[0].Tabela[i];
					}
					if (tabela[0].Grupo[0].Tabela[i].iPosicao == 10) {
						pos10 = tabela[0].Grupo[0].Tabela[i];
					}

					console.log(
						"6.2.1." +
							i +
							". - tabela.json tabela[0].Grupo[0].Tabela[i].iPosicao - Definiu as posições:",
					);
				...
				$(".fase0").append(
					"" +
						...
						"	</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-posicao">' +
						pos01.iPosicao +
						"º</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-escudo" style="background-image:url(' +
						pos01.sLogo +
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						pos01.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						pos01.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						pos01.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						pos01.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						pos01.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						pos01.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						pos01.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						pos01.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						pos01.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						pos01.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						pos01.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha">' +
			...
			if (tabela.length > 0 && tabela[1]) {
				...
				if (
					tabela[1].Grupo[0].Tabela[0].iPontos >=
					tabela[1].Grupo[0].Tabela[1].iPontos
				) {
				...
				if (
					tabela[1].Grupo[1].Tabela[0].iPontos >=
					tabela[1].Grupo[1].Tabela[1].iPontos
				) {
				...
				if (
					tabela[1].Grupo[2].Tabela[0].iPontos >=
					tabela[1].Grupo[2].Tabela[1].iPontos
				) {
				...
				if (
					tabela[1].Grupo[3].Tabela[0].iPontos >=
					tabela[1].Grupo[3].Tabela[1].iPontos
				) {
				...
				$(".fase1").append(
					"" +
						...
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						quartas_pos01.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						quartas_pos01.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						quartas_pos01.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						quartas_pos01.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						quartas_pos01.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						quartas_pos01.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						quartas_pos01.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						quartas_pos01.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						quartas_pos01.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						quartas_pos01.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						quartas_pos01.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha">' +
						'	<div class="principal-tabela-classificacao-linha-info-btn">' +
						...
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						quartas_pos02.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						quartas_pos02.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						quartas_pos02.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						quartas_pos02.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						quartas_pos02.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						quartas_pos02.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						quartas_pos02.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						quartas_pos02.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						quartas_pos02.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						quartas_pos02.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						quartas_pos02.iCartao_Verm +
						"</div>" +
						"</div>" +
						...
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						quartas_pos03.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						quartas_pos03.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						quartas_pos03.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						quartas_pos03.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						quartas_pos03.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						quartas_pos03.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						quartas_pos03.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						quartas_pos03.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						quartas_pos03.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						quartas_pos03.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						quartas_pos03.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha">' +
						'	<div class="principal-tabela-classificacao-linha-info-btn">' +
						...
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						quartas_pos04.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						quartas_pos04.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						quartas_pos04.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						quartas_pos04.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						quartas_pos04.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						quartas_pos04.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						quartas_pos04.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						quartas_pos04.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						quartas_pos04.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						quartas_pos04.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						quartas_pos04.iCartao_Verm +
						"</div>" +
						"</div>" +
						...
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						quartas_pos05.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						quartas_pos05.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						quartas_pos05.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						quartas_pos05.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						quartas_pos05.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						quartas_pos05.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						quartas_pos05.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						quartas_pos05.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						quartas_pos05.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						quartas_pos05.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						quartas_pos05.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha">' +
						'	<div class="principal-tabela-classificacao-linha-info-btn">' +
						...
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						quartas_pos06.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						quartas_pos06.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						quartas_pos06.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						quartas_pos06.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						quartas_pos06.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						quartas_pos06.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						quartas_pos06.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						quartas_pos06.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						quartas_pos06.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						quartas_pos06.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						quartas_pos06.iCartao_Verm +
						"</div>" +
						"</div>" +
						...
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						quartas_pos07.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						quartas_pos07.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						quartas_pos07.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						quartas_pos07.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						quartas_pos07.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						quartas_pos07.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						quartas_pos07.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						quartas_pos07.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						quartas_pos07.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						quartas_pos07.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						quartas_pos07.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha">' +
						'	<div class="principal-tabela-classificacao-linha-info-btn">' +
						...
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						quartas_pos08.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						quartas_pos08.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						quartas_pos08.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						quartas_pos08.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						quartas_pos08.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						quartas_pos08.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						quartas_pos08.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						quartas_pos08.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						quartas_pos08.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						quartas_pos08.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						quartas_pos08.iCartao_Verm +
						"</div>" +
						"</div>" +
						"",
				);
			...
			if (tabela.length > 1 && tabela[2]) {
				...
				//Start - Ordena as posições de acordo com a TAG iPosicao FASE DE TURNOS
				if (
					tabela[2].Grupo[0].Tabela[0].iPontos >=
					tabela[2].Grupo[0].Tabela[1].iPontos
				) {
				...
				$(".fase2").append(
					"" +
						...
						'	<div class="principal-tabela-classificacao-linha-info-posicao"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-escudo" style="background-image:url(' +
						descenso_pos01.sLogo +
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						descenso_pos01.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						descenso_pos01.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						descenso_pos01.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						descenso_pos01.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						descenso_pos01.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						descenso_pos01.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						descenso_pos01.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						descenso_pos01.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						descenso_pos01.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						descenso_pos01.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						descenso_pos01.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha">' +
						'	<div class="principal-tabela-classificacao-linha-info-btn">' +
						...
						'	<div class="principal-tabela-classificacao-linha-info-posicao"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-escudo" style="background-image:url(' +
						descenso_pos02.sLogo +
						')"></div>' +
						'	<div class="principal-tabela-classificacao-linha-info-nome">' +
						descenso_pos02.sNome_Clube +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-pontos">' +
						descenso_pos02.iPontos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-jogos">' +
						descenso_pos02.iJogos +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						descenso_pos02.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						descenso_pos02.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						descenso_pos02.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						descenso_pos02.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						descenso_pos02.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						descenso_pos02.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						descenso_pos02.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						descenso_pos02.iCartao_Verm +
						"</div>" +
						"</div>" +
						"",
				);
			...
			if (tabela.length > 2 && tabela[3]) {
				...
				//Start - Ordena as posições de acordo com a TAG iPosicao FASE DE TURNOS
				if (
					tabela[3].Grupo[0].Tabela[0].iPontos >=
					tabela[3].Grupo[0].Tabela[1].iPontos
				) {
				...
				if (
					tabela[3].Grupo[1].Tabela[0].iPontos >=
					tabela[3].Grupo[1].Tabela[1].iPontos
				) {
				...
				$(".fase3").append(
					"" +
						...
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						semi_pos01.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						semi_pos01.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						semi_pos01.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						semi_pos01.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						semi_pos01.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						semi_pos01.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						semi_pos01.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						semi_pos01.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha">' +
						'	<div class="principal-tabela-classificacao-linha-info-btn">' +
						...
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						semi_pos02.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						semi_pos02.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						semi_pos02.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						semi_pos02.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						semi_pos02.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						semi_pos02.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						semi_pos02.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						semi_pos02.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha"><div class="principal-jogos-barra-txt">GRUPO B</div></div>' +
						'<div class="principal-tabela-classificacao-linha">' +
						...
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						semi_pos03.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						semi_pos03.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						semi_pos03.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						semi_pos03.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						semi_pos03.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						semi_pos03.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						semi_pos03.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						semi_pos03.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha">' +
						'	<div class="principal-tabela-classificacao-linha-info-btn">' +
						...
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						semi_pos04.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						semi_pos04.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						semi_pos04.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						semi_pos04.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						semi_pos04.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						semi_pos04.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						semi_pos04.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						semi_pos04.iCartao_Verm +
						"</div>" +
						"</div>" +
						"",
				);
			...
			if (tabela.length > 3 && tabela[4]) {
				...
				//Start - Ordena as posições de acordo com a TAG iPosicao FASE DE TURNOS
				if (
					tabela[4].Grupo[0].Tabela[0].iPontos >=
					tabela[4].Grupo[1].Tabela[0].iPontos
				) {
				...
				$(".fase4").append(
					"" +
						...
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						final_pos01.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						final_pos01.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						final_pos01.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						final_pos01.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						final_pos01.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						final_pos01.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						final_pos01.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						final_pos01.iCartao_Verm +
						"</div>" +
						"</div>" +
						'<div class="principal-tabela-classificacao-linha">' +
						'	<div class="principal-tabela-classificacao-linha-info-btn">' +
						...
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-vitorias">' +
						final_pos02.iVitorias +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-empates">' +
						final_pos02.iEmpates +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-derrotas">' +
						final_pos02.iDerrotas +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-pro">' +
						final_pos02.iGols_Pro +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-gols-contra">' +
						final_pos02.iGols_Contra +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-saldo-de-gols">' +
						final_pos02.iSaldo_Gols +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-amarelos">' +
						final_pos02.iCartao_Amar +
						"</div>" +
						'	<div class="principal-tabela-classificacao-linha-info-cartoes-vermelhos">' +
						final_pos02.iCartao_Verm +
						"</div>" +
						"</div>" +
						"",
				);
			...
			console.log(
				"6.7. tabela.json - contador - A contagem foi modificada para o próximo passo => NUMBER:",
			);
			console.log(contador);
		},
	});
	...
	$.ajax({
		...
		success: function (retorno) {
			...
			console.log(
				"7. rodadas.json - rodadas - Identifica os dados das Rodadas => JSON:",
			);
			console.log(rodadas);

			console.log("7.0.1. rodadas.json - URL de retorno das Rodadas:");
			console.log(
				"http://egol.fcf.com.br/sesp/WsEgol.wso/Get_Jogos/JSON/debug?iAno=" +
					iAno +
					"&iCampeonato=" +
					iCampeonato,
			);
				...
				if (
					...
					if (
						...
						if (tabela_json == 1) {
							...
							$(".principal-tabela-carrossel").removeClass(
								"principal-tabela-carrossel-turnos",
							);
						}
						...
						$(".principal-jogos-subtitulo").click(function () {
							...
							console.log(
								"rodadas.json - ver jogos de hoje: select Quartas de Final - IDA",
							);
						});
					}
				}
				...
				if (
					...
					if (
						...
						$(".principal-jogos-subtitulo").click(function () {
							...
							console.log(
								"rodadas.json - ver jogos de hoje: select Quartas de Final - VOLTA",
							);
						});
					}
				}
			...
			if (rodadas[12]) {
				...
				if (rodadas[11].Jogos[0].sSumula == "SIM") {
			...
			if (rodadas[13]) {
				...
				if (rodadas[12].Jogos[0].sSumula == "SIM") {
					if (
						...
						$(".principal-jogos-subtitulo").click(function () {
							...
							console.log(
								"rodadas.json - ver jogos de hoje: select Semi Finais - IDA",
							);
						});
					}
				}
				...
				if (
					...
					if (
						...
						$(".principal-jogos-subtitulo").click(function () {
							...
							console.log(
								"rodadas.json - ver jogos de hoje: select Semi Finais - VOLTA",
							);
						});
					}
				}
			...
			if (rodadas[16]) {
				...
				if (rodadas[15].Jogos[0].sSumula == "SIM") {
			...
			if (rodadas[16]) {
				...
				if (rodadas[16].Jogos[0].sSumula == "SIM") {
					...
					$(".principal-jogos-subtitulo").click(function () {
						...
						console.log(
							"rodadas.json - ver jogos de hoje: select Este Campeonato está encerrado",
						);
					});
				}
			}
			...
			for (i = 0; i <= 8; i++) {
				...
				for (j = 0; j < rodadas[i].Jogos.length; j++) {
					...
					$(
						".rodada" +
							contador_rodada +
							" > .principal-jogos-barra > .principal-jogos-barra-txt",
					).html(contador_rodada + "ª RODADA");
					$(
						".rodada" +
							contador_rodada +
							" > .principal-jogos-barra > .principal-jogos-barra-txt-sub",
					).html("TURNOS");

					if (rodadas[i].Jogos[j].sNmMandante != "") {
						$(
							".rodada" + contador_rodada + " .jogo" + contador_jogo + " .jogo-data",
						).html(
							rodadas[i].Jogos[j].sDiaSemana +
								", " +
								rodadas[i].Jogos[j].sData +
								" " +
								rodadas[i].Jogos[j].sHora +
								" - Jogo: " +
								rodadas[i].Jogos[j].sJogo,
						);
						$(
							".rodada" +
								contador_rodada +
								" .jogo" +
								contador_jogo +
								" .jogo-time-casa-abrev",
						).html(rodadas[i].Jogos[j].sNmMandante);
						$(
							".rodada" +
								contador_rodada +
								" .jogo" +
								contador_jogo +
								" .jogo-time-casa-escudo",
						).css(
							"background-image",
							"url(" + rodadas[i].Jogos[j].sLogoMandante + ")",
						);
						$(
							".rodada" +
								contador_rodada +
								" .jogo" +
								contador_jogo +
								" .principal-jogos-resultado-gols",
						).html(
							rodadas[i].Jogos[j].iGolsMandante +
								" x " +
								rodadas[i].Jogos[j].iGolsVisitante,
						);
						$(
							".rodada" +
								contador_rodada +
								" .jogo" +
								contador_jogo +
								" .jogo-time-visitante-escudo",
						).css(
							"background-image",
							"url(" + rodadas[i].Jogos[j].sLogoVisitante + ")",
						);
						$(
							".rodada" +
								contador_rodada +
								" .jogo" +
								contador_jogo +
								" .jogo-time-visitante-abrev",
						).html(rodadas[i].Jogos[j].sNmVisitante);
						$(
							".rodada" +
								contador_rodada +
								" .jogo" +
							...
							rodadas[i].Jogos[j].sEstadio + " - " + rodadas[i].Jogos[j].sCidade,
						);
						if (
							...
							$(
								".rodada" +
									...
									" .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-transmissao",
							).append(
								"" +
									'	<div class="principal-jogos-resultado-transmissao-50" style="background-image:url(' +
									rodadas[i].Jogos[j].sLogoTransmissao +
									')"></div>' +
									'	<div class="principal-jogos-resultado-transmissao-50" style="background-image:url(' +
									rodadas[i].Jogos[j].sLogoTransmissao2 +
						...
						if (
							...
							$(
								...
								"" +
									'	<div class="principal-jogos-resultado-transmissao-100" style="background-image:url(' +
									rodadas[i].Jogos[j].sLogoTransmissao +
									')"></div>' +
									"",
							);
						...
						if (rodadas[i].Jogos[j].sSumula == "SIM") {
							$(
								".rodada" +
									...
									" .jogo" +
									contador_jogo +
									" .principal-jogos-btn-sumula",
							).append(
								"" +
									'<div class="principal-jogos-btn"><a href="sumula.html?sumula=1&iAno=' +
									rodadas[i].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[i].Jogos[j].sJogo +
									'" target="_blank">Súmula</a></div>' +
									"",
							);
						}
						if (rodadas[i].Jogos[j].sArbritagem == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="arbitragem.html?sumula=1&iAno=' +
									rodadas[i].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[i].Jogos[j].sJogo +
									'" target="_blank">Arbitragem</a></div>' +
									"",
							);
						}
						if (rodadas[i].Jogos[j].sDelegado == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="delegado.html?sumula=1&iAno=' +
									rodadas[i].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[i].Jogos[j].sJogo +
									'" target="_blank">Delegado</a></div>' +
									"",
							);
						}
						if (rodadas[i].Jogos[j].sBoletimFinan == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="boletim.html?sumula=1&iAno=' +
									rodadas[i].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[i].Jogos[j].sJogo +
									'" target="_blank">Bol. Financeiro</a></div>' +
									"",
							);
						}
						if (rodadas[i].Jogos[j].sIMT == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="imt.html?sumula=1&iAno=' +
									rodadas[i].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[i].Jogos[j].sJogo +
									'" target="_blank">IMT</a></div>' +
									"",
							);
						}
						if (rodadas[i].Jogos[j].sPortoesFechado == "SIM") {
							$(
								".rodada" +
									...
									" .jogo" +
									contador_jogo +
									" .principal-jogos-portoes-fechados",
							).addClass("principal-jogos-portoes-fechados-on");
						}
						...
						$(
							".rodada" + contador_rodada + " .jogo" + contador_jogo + " .jogo-data",
						).html("Jogo: " + rodadas[i].Jogos[j].sJogo);
						$(
							".rodada" +
								contador_rodada +
								" .jogo" +
						...
						$(
							".rodada" +
								...
								contador_jogo +
								" .jogo-time-casa-escudo",
						).addClass("jogo-a-decidir-escudo");
						$(
							".rodada" +
								contador_rodada +
								" .jogo" +
			...
			if (rodadas[9]) {
				...
				for (j = 0; j < rodadas[9].Jogos.length; j++) {
					...
					$(
						".rodadaQUARTAS_IDA > .principal-jogos-barra > .principal-jogos-barra-txt",
					).html("QUARTAS DE FINAL");
					$(
						".rodadaQUARTAS_IDA > .principal-jogos-barra > .principal-jogos-barra-txt-sub",
					).html("IDA");

					if (rodadas[9].Jogos[j].sNmMandante != "") {
						$(".rodadaQUARTAS_IDA .jogo" + contador_jogo + " .jogo-data").html(
							rodadas[9].Jogos[j].sDiaSemana +
								", " +
								rodadas[9].Jogos[j].sData +
								" " +
								rodadas[9].Jogos[j].sHora +
								" - Jogo: " +
								rodadas[9].Jogos[j].sJogo,
						);
						$(
							".rodadaQUARTAS_IDA .jogo" + contador_jogo + " .jogo-time-casa-abrev",
						).html(rodadas[9].Jogos[j].sNmMandante);
						$(
							".rodadaQUARTAS_IDA .jogo" + contador_jogo + " .jogo-time-casa-escudo",
						).css(
							"background-image",
							"url(" + rodadas[9].Jogos[j].sLogoMandante + ")",
						);
						if (rodadas[9].Jogos[j].iGolsMandantePen != 0) {
							$(
								".rodadaQUARTAS_IDA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-mandante",
							).html(rodadas[9].Jogos[j].iGolsMandantePen);
						}
						$(
							".rodadaQUARTAS_IDA .jogo" +
								contador_jogo +
							...
							rodadas[9].Jogos[j].iGolsMandante +
								" x " +
								rodadas[9].Jogos[j].iGolsVisitante,
						);
						if (rodadas[9].Jogos[j].iGolsVisitantePen != 0) {
							$(
								".rodadaQUARTAS_IDA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-visitante",
							).html(rodadas[9].Jogos[j].iGolsVisitantePen);
						}
						$(
							".rodadaQUARTAS_IDA .jogo" +
								contador_jogo +
							...
							"background-image",
							"url(" + rodadas[9].Jogos[j].sLogoVisitante + ")",
						);
						$(
							".rodadaQUARTAS_IDA .jogo" +
								contador_jogo +
								" .jogo-time-visitante-abrev",
						).html(rodadas[9].Jogos[j].sNmVisitante);
						$(".rodadaQUARTAS_IDA .jogo" + contador_jogo + " .jogo-estadio").html(
							rodadas[9].Jogos[j].sEstadio + " - " + rodadas[9].Jogos[j].sCidade,
						);
						if (
						...
						if (rodadas[9].Jogos[j].sSumula == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="sumula.html?sumula=1&iAno=' +
									rodadas[9].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[9].Jogos[j].sJogo +
					...
					console.log(
						"7.3. rodadas.json - Inseriu a rodada QUARTAS DE FINAL - IDA.",
					);
				}
			} else {
				for (j = 0; j < 5; j++) {
			...
			if (rodadas[10]) {
				...
				for (j = 0; j < rodadas[10].Jogos.length; j++) {
					...
					$(
						".rodadaQUARTAS_VOLTA > .principal-jogos-barra > .principal-jogos-barra-txt",
					).html("QUARTAS DE FINAL");
					$(
						".rodadaQUARTAS_VOLTA > .principal-jogos-barra > .principal-jogos-barra-txt-sub",
					).html("VOLTA");

					if (rodadas[10].Jogos[j].sNmMandante != "") {
						$(".rodadaQUARTAS_VOLTA .jogo" + contador_jogo + " .jogo-data").html(
							rodadas[10].Jogos[j].sDiaSemana +
								", " +
								rodadas[10].Jogos[j].sData +
								" " +
								rodadas[10].Jogos[j].sHora +
								" - Jogo: " +
								rodadas[10].Jogos[j].sJogo,
						);
						$(
							".rodadaQUARTAS_VOLTA .jogo" + contador_jogo + " .jogo-time-casa-abrev",
						).html(rodadas[10].Jogos[j].sNmMandante);
						$(
							".rodadaQUARTAS_VOLTA .jogo" + contador_jogo + " .jogo-time-casa-escudo",
						).css(
							"background-image",
							"url(" + rodadas[10].Jogos[j].sLogoMandante + ")",
						);
						if (rodadas[10].Jogos[j].iGolsMandantePen != 0) {
							$(
								".rodadaQUARTAS_VOLTA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-mandante",
							).html(rodadas[10].Jogos[j].iGolsMandantePen);
						}
						$(
							".rodadaQUARTAS_VOLTA .jogo" +
								contador_jogo +
							...
							rodadas[10].Jogos[j].iGolsMandante +
								" x " +
								rodadas[10].Jogos[j].iGolsVisitante,
						);
						if (rodadas[10].Jogos[j].iGolsVisitantePen != 0) {
							$(
								".rodadaQUARTAS_VOLTA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-visitante",
							).html(rodadas[10].Jogos[j].iGolsVisitantePen);
						}
						$(
							".rodadaQUARTAS_VOLTA .jogo" +
								contador_jogo +
							...
							"background-image",
							"url(" + rodadas[10].Jogos[j].sLogoVisitante + ")",
						);
						$(
							".rodadaQUARTAS_VOLTA .jogo" +
								contador_jogo +
								" .jogo-time-visitante-abrev",
						).html(rodadas[10].Jogos[j].sNmVisitante);
						$(".rodadaQUARTAS_VOLTA .jogo" + contador_jogo + " .jogo-estadio").html(
							rodadas[10].Jogos[j].sEstadio + " - " + rodadas[10].Jogos[j].sCidade,
						);
						if (rodadas[10].Jogos[j].sSumula == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="sumula.html?sumula=1&iAno=' +
									rodadas[10].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[10].Jogos[j].sJogo +
					...
					console.log(
						"7.4. rodadas.json - Inseriu a rodada QUARTAS DE FINAL - VOLTA.",
					);
				}
			} else {
				for (j = 0; j < 5; j++) {
			...
			if (rodadas[11]) {
				...
				for (j = 0; j < rodadas[11].Jogos.length; j++) {
					...
					$(
						".rodadaDESCENSO_IDA > .principal-jogos-barra > .principal-jogos-barra-txt",
					).html("DESCENSO");
					$(
						".rodadaDESCENSO_IDA > .principal-jogos-barra > .principal-jogos-barra-txt-sub",
					).html("IDA");

					if (rodadas[11].Jogos[j].sNmMandante != "") {
						$(".rodadaDESCENSO_IDA .jogo" + contador_jogo + " .jogo-data").html(
							rodadas[11].Jogos[j].sDiaSemana +
								", " +
								rodadas[11].Jogos[j].sData +
								" " +
								rodadas[11].Jogos[j].sHora +
								" - Jogo: " +
								rodadas[11].Jogos[j].sJogo,
						);
						$(
							".rodadaDESCENSO_IDA .jogo" + contador_jogo + " .jogo-time-casa-abrev",
						).html(rodadas[11].Jogos[j].sNmMandante);
						$(
							".rodadaDESCENSO_IDA .jogo" + contador_jogo + " .jogo-time-casa-escudo",
						).css(
							"background-image",
							"url(" + rodadas[11].Jogos[j].sLogoMandante + ")",
						);
						if (rodadas[11].Jogos[j].iGolsMandantePen != 0) {
							$(
								".rodadaDESCENSO_IDA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-mandante",
							).html(rodadas[11].Jogos[j].iGolsMandantePen);
						}
						$(
							".rodadaDESCENSO_IDA .jogo" +
								contador_jogo +
							...
							rodadas[11].Jogos[j].iGolsMandante +
								" x " +
								rodadas[11].Jogos[j].iGolsVisitante,
						);
						if (rodadas[11].Jogos[j].iGolsVisitantePen != 0) {
							$(
								".rodadaDESCENSO_IDA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-visitante",
							).html(rodadas[11].Jogos[j].iGolsVisitantePen);
						}
						$(
							".rodadaDESCENSO_IDA .jogo" +
								contador_jogo +
							...
							"background-image",
							"url(" + rodadas[11].Jogos[j].sLogoVisitante + ")",
						);
						$(
							".rodadaDESCENSO_IDA .jogo" +
								contador_jogo +
								" .jogo-time-visitante-abrev",
						).html(rodadas[11].Jogos[j].sNmVisitante);
						$(".rodadaDESCENSO_IDA .jogo" + contador_jogo + " .jogo-estadio").html(
							rodadas[11].Jogos[j].sEstadio + " - " + rodadas[11].Jogos[j].sCidade,
						);
						if (
						...
						if (rodadas[11].Jogos[j].sSumula == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="sumula.html?sumula=1&iAno=' +
									rodadas[11].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[11].Jogos[j].sJogo +
			...
			if (rodadas[12]) {
				...
				for (j = 0; j < rodadas[12].Jogos.length; j++) {
					...
					$(
						".rodadaDESCENSO_VOLTA > .principal-jogos-barra > .principal-jogos-barra-txt",
					).html("DESCENSO");
					$(
						".rodadaDESCENSO_VOLTA > .principal-jogos-barra > .principal-jogos-barra-txt-sub",
					).html("VOLTA");

					if (rodadas[12].Jogos[j].sNmMandante != "") {
						$(".rodadaDESCENSO_VOLTA .jogo" + contador_jogo + " .jogo-data").html(
							rodadas[12].Jogos[j].sDiaSemana +
								", " +
								rodadas[12].Jogos[j].sData +
								" " +
								rodadas[12].Jogos[j].sHora +
								" - Jogo: " +
								rodadas[12].Jogos[j].sJogo,
						);
						$(
							".rodadaDESCENSO_VOLTA .jogo" + contador_jogo + " .jogo-time-casa-abrev",
						).html(rodadas[12].Jogos[j].sNmMandante);
						$(
							".rodadaDESCENSO_VOLTA .jogo" +
								contador_jogo +
								" .jogo-time-casa-escudo",
						).css(
							"background-image",
							"url(" + rodadas[12].Jogos[j].sLogoMandante + ")",
						);
						if (rodadas[12].Jogos[j].iGolsMandantePen != 0) {
							$(
								".rodadaDESCENSO_VOLTA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-mandante",
							).html(rodadas[12].Jogos[j].iGolsMandantePen);
						}
						$(
							".rodadaDESCENSO_VOLTA .jogo" +
								contador_jogo +
							...
							rodadas[12].Jogos[j].iGolsMandante +
								" x " +
								rodadas[12].Jogos[j].iGolsVisitante,
						);
						if (rodadas[12].Jogos[j].iGolsVisitantePen != 0) {
							$(
								".rodadaDESCENSO_VOLTA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-visitante",
							).html(rodadas[12].Jogos[j].iGolsVisitantePen);
						}
						$(
							".rodadaDESCENSO_VOLTA .jogo" +
								contador_jogo +
							...
							"background-image",
							"url(" + rodadas[12].Jogos[j].sLogoVisitante + ")",
						);
						$(
							".rodadaDESCENSO_VOLTA .jogo" +
								contador_jogo +
								" .jogo-time-visitante-abrev",
						).html(rodadas[12].Jogos[j].sNmVisitante);
						$(".rodadaDESCENSO_VOLTA .jogo" + contador_jogo + " .jogo-estadio").html(
							rodadas[12].Jogos[j].sEstadio + " - " + rodadas[12].Jogos[j].sCidade,
						);
						if (
						...
						if (rodadas[12].Jogos[j].sSumula == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="sumula.html?sumula=1&iAno=' +
									rodadas[12].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[12].Jogos[j].sJogo +
				...
				for (j = 0; j < 2; j++) {
					...
					$(
						".rodadaDESCENSO_VOLTA .jogo" + contador_jogo + " .jogo-time-casa-escudo",
					).addClass("jogo-a-decidir-escudo");
					$(
						".rodadaDESCENSO_VOLTA .jogo" +
							contador_jogo +
			...
			if (rodadas[13]) {
				...
				for (j = 0; j < rodadas[13].Jogos.length; j++) {
					...
					$(
						".rodadaSEMI_IDA > .principal-jogos-barra > .principal-jogos-barra-txt",
					).html("SEMI FINAIS");
					$(
						".rodadaSEMI_IDA > .principal-jogos-barra > .principal-jogos-barra-txt-sub",
					).html("IDA");

					if (rodadas[13].Jogos[j].sNmMandante != "") {
						$(".rodadaSEMI_IDA .jogo" + contador_jogo + " .jogo-data").html(
							rodadas[13].Jogos[j].sDiaSemana +
								", " +
								rodadas[13].Jogos[j].sData +
								" " +
								rodadas[13].Jogos[j].sHora +
								" - Jogo: " +
								rodadas[13].Jogos[j].sJogo,
						);
						$(
							".rodadaSEMI_IDA .jogo" + contador_jogo + " .jogo-time-casa-abrev",
						).html(rodadas[13].Jogos[j].sNmMandante);
						$(
							".rodadaSEMI_IDA .jogo" + contador_jogo + " .jogo-time-casa-escudo",
						).css(
							"background-image",
							"url(" + rodadas[13].Jogos[j].sLogoMandante + ")",
						);
						if (rodadas[13].Jogos[j].iGolsMandantePen != 0) {
							$(
								".rodadaSEMI_IDA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-mandante",
							).html(rodadas[13].Jogos[j].iGolsMandantePen);
						}
						$(
							".rodadaSEMI_IDA .jogo" +
								contador_jogo +
							...
							rodadas[13].Jogos[j].iGolsMandante +
								" x " +
								rodadas[13].Jogos[j].iGolsVisitante,
						);
						if (rodadas[13].Jogos[j].iGolsVisitantePen != 0) {
							$(
								".rodadaSEMI_IDA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-visitante",
							).html(rodadas[13].Jogos[j].iGolsVisitantePen);
						}
						$(
							".rodadaSEMI_IDA .jogo" + contador_jogo + " .jogo-time-visitante-escudo",
						).css(
							"background-image",
							"url(" + rodadas[13].Jogos[j].sLogoVisitante + ")",
						);
						$(
							".rodadaSEMI_IDA .jogo" + contador_jogo + " .jogo-time-visitante-abrev",
						).html(rodadas[13].Jogos[j].sNmVisitante);
						$(".rodadaSEMI_IDA .jogo" + contador_jogo + " .jogo-estadio").html(
							rodadas[13].Jogos[j].sEstadio + " - " + rodadas[13].Jogos[j].sCidade,
						);
						if (
						...
						if (rodadas[13].Jogos[j].sSumula == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="sumula.html?sumula=1&iAno=' +
									rodadas[13].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[13].Jogos[j].sJogo +
				...
				for (j = 0; j < 3; j++) {
					...
					$(".rodadaSEMI_IDA .jogo" + contador_jogo + " .jogo-time-casa-abrev").html(
						"MANDANTE",
					);
					$(
						".rodadaSEMI_IDA .jogo" + contador_jogo + " .jogo-time-casa-escudo",
			...
			if (rodadas[14]) {
				...
				for (j = 0; j < rodadas[14].Jogos.length; j++) {
					...
					$(
						".rodadaSEMI_VOLTA > .principal-jogos-barra > .principal-jogos-barra-txt",
					).html("SEMI FINAIS");
					$(
						".rodadaSEMI_VOLTA > .principal-jogos-barra > .principal-jogos-barra-txt-sub",
					).html("VOLTA");

					if (rodadas[14].Jogos[j].sNmMandante != "") {
						$(".rodadaSEMI_VOLTA .jogo" + contador_jogo + " .jogo-data").html(
							rodadas[14].Jogos[j].sDiaSemana +
								", " +
								rodadas[14].Jogos[j].sData +
								" " +
								rodadas[14].Jogos[j].sHora +
								" - Jogo: " +
								rodadas[14].Jogos[j].sJogo,
						);
						$(
							".rodadaSEMI_VOLTA .jogo" + contador_jogo + " .jogo-time-casa-abrev",
						).html(rodadas[14].Jogos[j].sNmMandante);
						$(
							".rodadaSEMI_VOLTA .jogo" + contador_jogo + " .jogo-time-casa-escudo",
						).css(
							"background-image",
							"url(" + rodadas[14].Jogos[j].sLogoMandante + ")",
						);
						if (rodadas[14].Jogos[j].iGolsMandantePen != 0) {
							$(
								".rodadaSEMI_VOLTA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-mandante",
							).html(rodadas[14].Jogos[j].iGolsMandantePen);
						}
						$(
							".rodadaSEMI_VOLTA .jogo" +
								contador_jogo +
							...
							rodadas[14].Jogos[j].iGolsMandante +
								" x " +
								rodadas[14].Jogos[j].iGolsVisitante,
						);
						if (rodadas[14].Jogos[j].iGolsVisitantePen != 0) {
							$(
								".rodadaSEMI_VOLTA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-visitante",
							).html(rodadas[14].Jogos[j].iGolsVisitantePen);
						}
						$(
							".rodadaSEMI_VOLTA .jogo" +
								contador_jogo +
							...
							"background-image",
							"url(" + rodadas[14].Jogos[j].sLogoVisitante + ")",
						);
						$(
							".rodadaSEMI_VOLTA .jogo" +
								contador_jogo +
								" .jogo-time-visitante-abrev",
						).html(rodadas[14].Jogos[j].sNmVisitante);
						$(".rodadaSEMI_VOLTA .jogo" + contador_jogo + " .jogo-estadio").html(
							rodadas[14].Jogos[j].sEstadio + " - " + rodadas[14].Jogos[j].sCidade,
						);
						if (
						...
						if (rodadas[14].Jogos[j].sSumula == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="sumula.html?sumula=1&iAno=' +
									rodadas[14].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[14].Jogos[j].sJogo +
				...
				for (j = 0; j < 3; j++) {
					...
					$(
						".rodadaSEMI_VOLTA .jogo" + contador_jogo + " .jogo-time-visitante-abrev",
					).html("VISITANTE");
					$(".rodadaSEMI_VOLTA .jogo" + contador_jogo + " .jogo-estadio").html(
						"Local a decidir",
					);
			...
			if (rodadas[15]) {
				...
				for (j = 0; j < rodadas[15].Jogos.length; j++) {
					...
					$(
						".rodadaFINAIS_IDA > .principal-jogos-barra > .principal-jogos-barra-txt",
					).html("FINAIS");
					$(
						".rodadaFINAIS_IDA > .principal-jogos-barra > .principal-jogos-barra-txt-sub",
					).html("IDA");

					if (rodadas[15].Jogos[j].sNmMandante != "") {
						$(".rodadaFINAIS_IDA .jogo" + contador_jogo + " .jogo-data").html(
							rodadas[15].Jogos[j].sDiaSemana +
								", " +
								rodadas[15].Jogos[j].sData +
								" " +
								rodadas[15].Jogos[j].sHora +
								" - Jogo: " +
								rodadas[15].Jogos[j].sJogo,
						);
						$(
							".rodadaFINAIS_IDA .jogo" + contador_jogo + " .jogo-time-casa-abrev",
						).html(rodadas[15].Jogos[j].sNmMandante);
						$(
							".rodadaFINAIS_IDA .jogo" + contador_jogo + " .jogo-time-casa-escudo",
						).css(
							"background-image",
							"url(" + rodadas[15].Jogos[j].sLogoMandante + ")",
						);
						if (rodadas[15].Jogos[j].iGolsMandantePen != 0) {
							$(
								".rodadaFINAIS_IDA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-mandante",
							).html(rodadas[15].Jogos[j].iGolsMandantePen);
						}
						$(
							".rodadaFINAIS_IDA .jogo" +
								contador_jogo +
							...
							rodadas[15].Jogos[j].iGolsMandante +
								" x " +
								rodadas[15].Jogos[j].iGolsVisitante,
						);
						if (rodadas[15].Jogos[j].iGolsVisitantePen != 0) {
							$(
								".rodadaFINAIS_IDA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-visitante",
							).html(rodadas[15].Jogos[j].iGolsVisitantePen);
						}
						$(
							".rodadaFINAIS_IDA .jogo" +
								contador_jogo +
							...
							"background-image",
							"url(" + rodadas[15].Jogos[j].sLogoVisitante + ")",
						);
						$(
							".rodadaFINAIS_IDA .jogo" +
								contador_jogo +
								" .jogo-time-visitante-abrev",
						).html(rodadas[15].Jogos[j].sNmVisitante);
						$(".rodadaFINAIS_IDA .jogo" + contador_jogo + " .jogo-estadio").html(
							rodadas[15].Jogos[j].sEstadio + " - " + rodadas[15].Jogos[j].sCidade,
						);
						if (
						...
						if (rodadas[15].Jogos[j].sSumula == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="sumula.html?sumula=1&iAno=' +
									rodadas[15].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[15].Jogos[j].sJogo +
				...
				for (j = 0; j < 2; j++) {
					...
					$(
						".rodadaFINAIS_IDA .jogo" + contador_jogo + " .jogo-time-visitante-abrev",
					).html("VISITANTE");
					$(".rodadaFINAIS_IDA .jogo" + contador_jogo + " .jogo-estadio").html(
						"Local a decidir",
					);
			...
			if (rodadas[16]) {
				...
				for (j = 0; j < rodadas[16].Jogos.length; j++) {
					...
					$(
						".rodadaFINAIS_VOLTA > .principal-jogos-barra > .principal-jogos-barra-txt",
					).html("FINAIS");
					$(
						".rodadaFINAIS_VOLTA > .principal-jogos-barra > .principal-jogos-barra-txt-sub",
					).html("VOLTA");

					if (rodadas[16].Jogos[j].sNmMandante != "") {
						$(".rodadaFINAIS_VOLTA .jogo" + contador_jogo + " .jogo-data").html(
							rodadas[16].Jogos[j].sDiaSemana +
								", " +
								rodadas[16].Jogos[j].sData +
								" " +
								rodadas[16].Jogos[j].sHora +
								" - Jogo: " +
								rodadas[16].Jogos[j].sJogo,
						);
						$(
							".rodadaFINAIS_VOLTA .jogo" + contador_jogo + " .jogo-time-casa-abrev",
						).html(rodadas[16].Jogos[j].sNmMandante);
						$(
							".rodadaFINAIS_VOLTA .jogo" + contador_jogo + " .jogo-time-casa-escudo",
						).css(
							"background-image",
							"url(" + rodadas[16].Jogos[j].sLogoMandante + ")",
						);
						if (rodadas[16].Jogos[j].iGolsMandantePen != 0) {
							$(
								".rodadaFINAIS_VOLTA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-mandante",
							).html(rodadas[16].Jogos[j].iGolsMandantePen);
						}
						$(
							".rodadaFINAIS_VOLTA .jogo" +
								contador_jogo +
							...
							rodadas[16].Jogos[j].iGolsMandante +
								" x " +
								rodadas[16].Jogos[j].iGolsVisitante,
						);
						if (rodadas[16].Jogos[j].iGolsVisitantePen != 0) {
							$(
								".rodadaFINAIS_VOLTA .jogo" +
									contador_jogo +
									" .principal-jogos-resultado-penalti-visitante",
							).html(rodadas[16].Jogos[j].iGolsVisitantePen);
						}
						$(
							".rodadaFINAIS_VOLTA .jogo" +
								contador_jogo +
							...
							"background-image",
							"url(" + rodadas[16].Jogos[j].sLogoVisitante + ")",
						);
						$(
							".rodadaFINAIS_VOLTA .jogo" +
								contador_jogo +
								" .jogo-time-visitante-abrev",
						).html(rodadas[16].Jogos[j].sNmVisitante);
						$(".rodadaFINAIS_VOLTA .jogo" + contador_jogo + " .jogo-estadio").html(
							rodadas[16].Jogos[j].sEstadio + " - " + rodadas[16].Jogos[j].sCidade,
						);
						if (
						...
						if (rodadas[16].Jogos[j].sSumula == "SIM") {
							$(
								...
								"" +
									'<div class="principal-jogos-btn"><a href="sumula.html?sumula=1&iAno=' +
									rodadas[16].Jogos[j].sData.substr(6, 4) +
									"&iCampeonato=" +
									iCampeonato +
									"&iJogo=" +
									rodadas[16].Jogos[j].sJogo +
			...
			console.log(
				"7.7. rodadas.json - contador - A contagem foi modificada para o próximo passo => NUMBER:",
			);
			console.log(contador);
		},
	});
	...
	$.ajax({
		...
		success: function (retorno) {
			...
			console.log(
				"http://egol.fcf.com.br/sesp/WsEgol.wso/Get_Artilharia/JSON/debug?iAno=" +
					iAno +
					"&iCampeonato=" +
					iCampeonato,
			);
			...
			for (i = 0; i <= 5; i++) {
				...
				$(
					".artilheiro" +
						contador_artilheiro +
						" > .estatisticas-tabela-linha-body-clube",
				).css("background-image", "url(" + artilharia[i].sLogo + ")");
				$(
					".artilheiro" +
						contador_artilheiro +
						" > .estatisticas-tabela-linha-body-gols",
				).html(artilharia[i].iGols);
				$(
					".artilheiro" +
						contador_artilheiro +
						" > .estatisticas-tabela-linha-body-foto",
				).css("background-image", "url(" + artilharia[i].sFoto + ")");
				$(
					".artilheiro" +
						contador_artilheiro +
						" > .estatisticas-tabela-linha-body-nome",
				).html(artilharia[i].sNmJogador);
				$(
					".artilheiro" +
						contador_artilheiro +
						" > .estatisticas-tabela-linha-body-apelido",
				).html(artilharia[i].sApelido);
			}
	...
	$.ajax({
		...
		success: function (retorno) {
			...
			console.log(
				"http://egol.fcf.com.br/sesp/WsEgol.wso/Get_GoleiroMenosVazado/JSON/debug?iAno=" +
					iAno +
					"&iCampeonato=" +
					iCampeonato,
			);
			...
			for (i = 0; i < 5; i++) {
				...
				$(
					".goleiro" +
						contador_goleiro +
						" > .estatisticas-tabela-completa-linha-body-clube",
				).css("background-image", "url(" + goleiro[i].sLogo + ")");
				$(
					".goleiro" +
						contador_goleiro +
						" > .estatisticas-tabela-completa-linha-body-gols",
				).html(goleiro[i].iGols);
				$(
					".goleiro" +
						contador_goleiro +
						" > .estatisticas-tabela-completa-linha-body-foto",
				).css("background-image", "url(" + goleiro[i].sFoto + ")");
				$(
					".goleiro" +
						contador_goleiro +
						" > .estatisticas-tabela-completa-linha-body-nome",
				).html(goleiro[i].sNmJogador);
				$(
					".goleiro" +
						contador_goleiro +
						" > .estatisticas-tabela-completa-linha-body-apelido",
				).html(goleiro[i].sApelido);
				$(".goleiro" + contador_goleiro + " > .goleiro-media").html(
					goleiro[i].nMedia.toFixed(2),
				);
				$(".goleiro" + contador_goleiro + " > .goleiro-part").html(
					goleiro[i].nParticipacao.toFixed(2) + "%",
				);
				$(".goleiro" + contador_goleiro + " > .goleiro-atuacoes").html(
					goleiro[i].iAtuacoes,
				);
				$(".goleiro" + contador_goleiro + " > .goleiro-jogos").html(
					goleiro[i].iTotalJogos,
				);
			}
				...
				if (documentos[i].sNmDoc == "REGULAMENTO GERAL") {
					$(".documentos-reg-geral-campo").append(
						"" +
							...
							'	<div class="documentos-tela-tabela-linha-body-data">' +
							documentos[i].sTpDoc +
							"</div>" +
							'	<div class="documentos-tela-tabela-linha-body-titulo">' +
							documentos[i].sNmDoc +
							"</div>" +
							'	<div class="documentos-tela-tabela-linha-body-btn-espaco">' +
							'		<div class="documentos-tela-tabela-linha-body-btn"><a href="' +
							documentos[i].sPathDoc +
							'" target="_blank">Visualizar</a></div>' +
							"	</div>" +
							"</div>" +
							"",
					);
		...
		success: function (retorno) {
			...
			for (i = 0; i < clubes.length; i++) {
				...
				$(".documentos-ficha-campo").append(
					"" +
						'	<div class="documentos-tela-tabela-linha-clube documentos-clubes' +
						i +
				...
				if (i == 0) {
					$.ajax({
						...
						success: function (retorno) {
							...
							$(".time0").append(
								"" +
									'<div class="inscricoes-time-escudo" style="background-image:url(' +
									inscricoes.sLogo +
									')"></div>' +
									'<div class="inscricoes-time-nome">' +
									inscricoes.sNmClube +
									"</div>" +
									'<div class="inscricoes-time-numero">Inscrição do Clube: ' +
									inscricoes.sCodClube +
									"</div>" +
							...
							for (j = 0; j < inscricoes.Jogador.length; j++) {
								$(".time0 .inscricoes-time-tabela tbody").append(
									"" +
										"<tr>" +
										"	<td>" +
										inscricoes.Jogador[j].iSequencia +
										"</td>" +
										"	<td>" +
										inscricoes.Jogador[j].sCBF +
										"</td>" +
										'	<td><div class="inscricoes-time-tabela-foto" style="background-image:url(' +
										inscricoes.Jogador[j].sFoto +
										')" ></div>' +
										inscricoes.Jogador[j].sNmJogador +
										"</td>" +
										"	<td>" +
										inscricoes.Jogador[j].sCategoria +
										"</td>" +
										"	<td>" +
										inscricoes.Jogador[j].sDtInscricao +
										"</td>" +
										"	<td>" +
										inscricoes.Jogador[j].sAtestado +
										"</td>" +
										"</tr>" +
										+"",
								);
				...
				if (i == 9) {
					/*A TELA DE LOADING FINALIZA AQUI CASO TUDO ESTEJA PRONTO*/
